...
from crewai_payclaw.client import DEFAULT_BASE_URL, PayClawError
...
def test_pay_tool_happy_path():
    ...
    tool = PayClawPayTool()
    out = tool._run(to="0x" + "1" * 40, amount="0.05")
    assert json.loads(out)["txHash"] == SAMPLE_RECEIPT["txHash"]


def test_pay_tool_args_schema_rejects_bad_address():
    """The Pydantic args_schema validates inputs at agent-call time."""
    from crewai_payclaw.tools import _PayInput

    with pytest.raises(Exception, match="0x-prefixed"):
        _PayInput(to="not-an-address", amount="0.05")


def test_pay_tool_args_schema_rejects_bad_amount():
    from crewai_payclaw.tools import _PayInput

    with pytest.raises(Exception, match="decimal string"):
        _PayInput(to="0x" + "1" * 40, amount="five dollars")
