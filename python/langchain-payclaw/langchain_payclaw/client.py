"""Thin async/sync HTTP client for the PayClaw REST API.

The package exposes high-level LangChain tools in :mod:`langchain_payclaw.tools`
that wrap this client. Most users won't touch ``PayClawClient`` directly, but
it's exposed so advanced users can build custom tools, retries, or test fakes.
"""
...
class PayClawClient:
    ...
    async def apay(self, to: str, amount: str) -> dict[str, Any]:
        async with httpx.AsyncClient(timeout=self._timeout) as client:
            r = await client.post(
                f"{self._base_url}/api/gpt/pay",
                headers=self._headers,
    ...
    async def aget_history(self, limit: int = 10) -> dict[str, Any]:
        async with httpx.AsyncClient(timeout=self._timeout) as client:
            r = await client.get(
                f"{self._base_url}/api/gpt/history",
                headers=self._headers,
