...
> **Give your Microsoft AutoGen `AssistantAgent` a USDC wallet on Base it can actually spend.**
...
[Microsoft AutoGen](https://microsoft.github.io/autogen/) (v0.4+) that let any
...
If you prefer explicit tool registration (with custom names or descriptions),
use the pre-wrapped instances:
...
Every existing way to give an AutoGen agent a wallet has the same defect: it
needs ETH for gas. So you end up running an ETH-funding cron, or you bail and
use a custodial API and lose the whole point of agent-owned funds.
...
- Python ≥ 3.10
- `autogen-core >= 0.4.0` (required)
- `autogen-agentchat >= 0.4.0` (optional, install via `[agentchat]` extra)
- `pydantic >= 2.7`
- `httpx >= 0.27`
